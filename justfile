# set windows-shell := ["C:\\Program Files\\Git\\bin\\sh.exe","-c"]
set windows-shell := ["powershell.exe", "-NoLogo", "-Command"]


# list commands also default
list:
    @just --list

sqlx:
    cd sqlx_storage; cargo sqlx database reset -D sqlite:///$PWD/sqlx.sqlite -y
    cd sqlx_storage; cargo sqlx prepare -D sqlite:///$PWD/sqlx.sqlite -- -p openmls_sqlx_storage
    cd sqlx_storage; cargo sqlx database drop -D sqlite:///$PWD/sqlx.sqlite -y
    cargo check -p openmls_sqlx_storage
    cargo check -p svalin; echo "ignore error"
    cd svalin; cargo sqlx database reset -D sqlite:///$PWD/sqlx.sqlite -y
    cd svalin; cargo sqlx prepare -D sqlite:///$PWD/sqlx.sqlite -- -p svalin
    cd svalin; cargo sqlx database drop -D sqlite:///$PWD/sqlx.sqlite -y

test $RUST_LOG="svalin=trace":
    cargo test -p svalin_pki -p svalin_rpc -p svalin -- --nocapture

integration $RUST_LOG="svalin=trace":
    cargo test -p svalin test::integration -- --nocapture

server $RUST_LOG="svalin=trace":
    cargo run -p svalin server 0.0.0.0:55411

reset:
    rm -r /var/lib/svalin/*

reset-win:
    Remove-Item -Path C:\ProgramData\svalin\* -Recurse -Force

agent $RUST_LOG="svalin=trace":
    cargo run -p svalin agent run --profile development

agent_install $RUST_LOG="svalin=trace":
    cargo build --release -p svalin
    sudo target/release/svalin agent install

agent_uninstall $RUST_LOG="svalin=trace":
    cargo build --release -p svalin
    sudo target/release/svalin agent uninstall

agent_init $RUST_LOG="svalin=trace":
    cargo run -p svalin agent init localhost:55411

gui $RUST_LOG="svalin=trace":
    cd svalin_iced; cargo run

build-debian: build-cross
    cargo deb --package svalin --target x86_64-unknown-linux-gnu --no-build


build-cross:
    cross build --release
