...
impl RpcServer {
    ...
    async fn serve(self: Arc<Self>, commands: HandlerCollection<impl PermissionHandler>) {
        ...
        loop {
            // tracing::trace!("Waiting for next connection");
            select! {
                _ = self.config.cancellation_token.cancelled() => {
                    // tracing::trace!("canceling RPC main serve loop");
                    break;
                }
                conn_option = self.endpoint.accept() => {
                    match conn_option {
                        None => {
                            // tracing::trace!("no more connections");
                        },
                        Some(conn) => {
                            // tracing::trace!("connection incoming");
    ...
    async fn serve_connection_inner(
        ...
        // tracing::trace!("spawned new task for incoming connection");
        // tracing::trace!("waiting for connection to get ready...");
        ...
        if let Peer::Certificate(cert) = conn.peer() {
            // tracing::trace!("noting down connection for peer");
        ...
        // tracing::trace!("connection handled");

        Ok(())
    }

    async fn update_connection_data_on_close(self: Arc<Self>, conn: DirectConnection) {
        // tracing::trace!("spawned task to update connection data on close");
        conn.closed().await;
        if let Peer::Certificate(cert) = conn.peer() {
            // tracing::trace!("removing connection data for peer after close");
            let mut lock = self.connection_data.lock().await;
